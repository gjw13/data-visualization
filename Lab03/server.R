# To Deploy App
# rsconnect::setAccountInfo(name='ericyu',token='E710045E94EEA992C9A520B8C9607F61',secret='QQFjLzAbLttWi/4OsFhDlaPc1W4HQuZn36EfK9Oj')
# library(rsconnect)
# rsconnect::deployApp('/Users/EricYu/Desktop/R\ Programs/Data\ Visualization\ Spring\ 2019/Group\ 11\ Lab\ 03')
...
shinyServer(function(input, output) {
  ...
  ## ------------------------------------------------------------------------
  ## GRAPH 1 - Line Graph of Temperature Anomalies by Year - ge04
  ## ------------------------------------------------------------------------
  ...
  ## ------------------------------------------------------------------------
  ## GRAPH 3 - Bar Graph - Carbon Emissions by Year - ge04
  ## ------------------------------------------------------------------------
  ...
  output$barPlot <- renderPlot({
    ...
    #####parameterization##############
    xVar <- "Year"
    yVar <- "Cumulative CO2 emissions by nation (tonnes)"
    xLabel <- "Year"
    yLabel <- "CO2 World Emissions (in tons)"
    ###################################
    
    carbon_data <- filter(carbon_data,Year>=input$Year)
    
    xlimits <- c(1, max(carbon_data[,xVar],na.rm=T))
    ylimits <- c(1, max(carbon_data[,yVar],na.rm=T))
    midpoint <- c(mean(xlimits),mean(ylimits))
    
    mid<-mean(carbon_data$`Cumulative CO2 emissions by nation (tonnes)`)
    
    graph3 <- ggplot(carbon_data,
                     aes(x=Year,
                         y=`Cumulative CO2 emissions by nation (tonnes)`,
                         group=Year,
                         color=`Cumulative CO2 emissions by nation (tonnes)`))+
      geom_bar(stat="identity")+
      labs(x=xLabel,y=yLabel,title=paste("World CO2 Emissions by Year\n\n From", input$Year,"to 2016"))+
